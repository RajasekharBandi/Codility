/*
    BinaryGap
    - Find longest sequence of zeros in binary representation of an integer.
    - Difficulty Level: PAINLESS
   A binary gap within a positive integer N is any maximal sequence of consecutive zeros that is surrounded by ones at both ends
    in the binary representation of N.


For example, number 9 has binary representation 1001 and contains a binary gap of length 2. 
The number 529 has binary representation 1000010001 and contains two binary gaps: one of length 4 and one of length 3. 
The number 20 has binary representation 10100 and contains one binary gap of length 1. 
The number 15 has binary representation 1111 and has no binary gaps. 
The number 32 has binary representation 100000 and has no binary gaps.

Write a function:

class Solution { public int solution(int N); }

that, given a positive integer N, returns the length of its longest binary gap. 
The function should return 0 if N doesn't contain a binary gap.

For example, given N = 1041 the function should return 5, because N has binary representation 10000010001 
and so its longest binary gap is of length 5. Given N = 32 the function should return 0,
 because N has binary representation '100000' and thus no binary gaps.

Assume that:

N is an integer within the range [1..2,147,483,647].
 */

import java.util.Arrays;

class Solution {
	public static void main(String arg[])
	{
		Solution.solution2(32);
		Solution.solution2(529);
		Solution.solution2(20);
		Solution.solution2(15);
		Solution.solution2(32);
	}
       public static int solution2(int N){
    	
        String binaryString=Integer.toBinaryString(N);
        System.out.println(N+"-->Binary:"+binaryString);
        String bsStrip=binaryString.substring(0,binaryString.lastIndexOf("1"));
        System.out.println(bsStrip);
        String[] bsStrArr=bsStrip.split("1");
        int count=0;
      for(int i=0;i<bsStrArr.length;i++)
      {
    	  int len=bsStrArr[i].length();
    	  if(len>count)
    		  count=len;
    	  System.out.println("bsStrArr[i]:"+bsStrArr[i]+"; len:"+bsStrArr[i].length());
      }
      System.out.println("Largest Gap:"+count);
      
      
        if(bsStrArr.length!=0){
            count =Arrays.stream(bsStrArr).mapToInt(i->i.length()).max().getAsInt();
        }
        System.out.println("Using Arrays Stream:"+count);
        return count;
    }
       
       public static int solution3(int n) {//Codility-solution

   		int longestSeq = 0;
   		int currentSeq = 0;
   		boolean oneFlag = false;

   		String binaryN = Integer.toBinaryString(n);
   		for (char ch : binaryN.toCharArray()) {

   			if (oneFlag && ch == '0') {
   				currentSeq++;
   			}
   			else if (ch == '1') {

   				if (oneFlag) {
   					if (currentSeq > longestSeq) {
   						longestSeq = currentSeq;
   					}
   					currentSeq = 0;
   				}
   				oneFlag = true;
   			}
   		}
   		return longestSeq;
   	}
    
       public static int solution4(int N) {//Codility-solutions

           String binaryRepresentation = Integer.toBinaryString(N);
           int biggestGap = 0;
           int lastGap = 0;

           for (char bit : binaryRepresentation.toCharArray()) {
               if (bit == '0') {
                   lastGap++;
               } else {
                   if (lastGap > biggestGap) {
                       biggestGap = lastGap;
                   }
                   lastGap = 0;
               }
           }
           return biggestGap;
       }

   }